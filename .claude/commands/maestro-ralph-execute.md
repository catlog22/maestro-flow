...
1. **Load step via CLI** — `Bash("maestro ralph next", run_in_background: false)` （前台同步执行；stdout 即完整 prompt 块）
...
3. **Confirm completion via CLI** — 末尾调用 CLI 写入完成状态（**不再使用 `--- COMPLETION STATUS ---` 文本块**）：
   - 成功 → `Bash("maestro ralph complete N --status DONE [--evidence <path>]")`
   - 有遗留问题 → `Bash("maestro ralph complete N --status DONE_WITH_CONCERNS --concerns \"...\"")`
   - 工具/参数错误待重试 → `Bash("maestro ralph retry N")`
   - 外部阻塞（缺依赖/被占用） → `Bash("maestro ralph complete N --status BLOCKED --reason \"...\"")`
...
完成后：S_LOCATE 触发 `Skill({ skill: "maestro-ralph-execute" })` 自调用，由 CLI 一致性校验决定推进或暂停。
