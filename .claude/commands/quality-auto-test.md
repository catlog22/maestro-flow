...
**Output boundary**: ALL file writes MUST target `{run_dir}/outputs/` or `.workflow/state.json` only. Source code modifications are limited to test files generated by this command. NEVER modify existing production source code.
</context>
...
**Artifact registration:** Write the declared typed outputs under `{run_dir}/outputs/`. `maestro run complete` discovers, validates, hashes, and registers them automatically; the model never edits an artifact registry.
