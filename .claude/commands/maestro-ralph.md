...
> **执行模型**：每个 step 由 [@subagent] Agent(ralph-executor) 派发执行，非主会话内联。Agent 内部调 `maestro ralph next` 获取 skill prompt 并执行，结果通过 task-notification 回传主流程。
...
7. [@subagent] Agent() 返回 agentId → 等待 task-notification（status=completed 时 `<result>` 含 executor 输出）
