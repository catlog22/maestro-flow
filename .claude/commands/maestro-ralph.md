...
`{session_dir}` = `.workflow/.maestro/ralph-{YYYYMMDD-HHmmss}/`（ralph 编排目录，非工作单元 `.workflow/sessions/{id}/`）
</purpose>
...
- `.workflow/.maestro/ralph-*/status.json` — session state
</context>
...
22. **引擎只做并行加速，不做状态决策** — `--engine swarm|universal` 通过 Workflow 引擎并行执行单个 step，MUST NOT 修改 `status.json`、MUST NOT 推进 step、MUST NOT 触碰 decision 节点；引擎产出写入该 step 的 Run output dir（格式兼容对应命令产物），由主流程照常 `ralph complete`。生成/固定脚本对引擎只读（`wf-*.js` 从不被编辑；`uwf-*.js` 仅由 universal 生成器按幂等命名覆盖）。
</invariants>
...
> **执行模型**：每个 step 由 Agent(ralph-executor) 派发执行，非主会话内联。Agent 内部调 `maestro ralph next` 获取 skill prompt 并执行，结果通过 task-notification 回传主流程。
...
**控制权边界**：两个引擎均为**并行加速器，非状态决策者** —— 从不修改 ralph `status.json`、从不推进 step。FSM 保留 session 生命周期 + step 排序的所有权（invariant 21 控制权优先级）。引擎调用 `Workflow` 工具在 step 内部并行执行，结果回填该 step 的产物目录，仍由主流程 A_STEP_COMPLETE 调 `ralph complete` 上报。
...
**Invariants:**
- Parallel-accelerate only —— 从不修改 ralph status.json，从不推进 step。
...
```json
{
  "ralph_session_id": "ralph-{YYYYMMDD-HHmmss}",
  ...
  "session_id": null, "session_is_new": false,
  "auto_mode": false,
...
- [ ] 两引擎均不修改 ralph `status.json` 或推进 step（控制权优先级 invariant 21 不变）
