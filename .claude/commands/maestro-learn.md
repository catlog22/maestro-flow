...
- `--save-spec`: `[@skill] Skill("maestro-spec")` (add subcommand) for each new pattern
...
**Pre-load** (optional): `[@skill] Skill("maestro-spec")` (load subcommand) for conventions + `maestro search "<target topic>"` for related entries.
