...
| `architecture_template_evidence` | optional | Loaded reference-only templates; used only by system-architect |
...
5. For system-architect only, evaluate supplied candidates and cite template IDs for adopted/adapted positions. Do not treat template text as project truth. Other roles preserve but do not independently reinterpret this evidence.
6. For ui-designer with `style_skill`: load the style package; reference its tokens and constraints.
7. Write `analysis.md` first — this is the index. Build §2 Decision Digest as you analyze.
8. Write one `analysis-F-{id}-{slug}.md` per feature. Keep each focused and < 2000 words.
9. Write `findings-{slug}.md` for any discoveries outside the feature scope. Skip if none.
10. After all files are written, finalize §4 File Index in `analysis.md` with accurate headings from the written files.
