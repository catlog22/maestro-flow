...
1. Resolve the Run — **全量捕获 stdout，严禁截断管道**：
   ...
   - dispatch prompt 缺 `run_id` → 返回 `BLOCKED: missing authoritative run next birth packet`；不得调用不完整或猜测 authority 的 `run next`
   - birth packet 的 resolved `task.command` / positional `task.args` / `task.goal` / `task.input_refs` 是执行契约；structured `continuation` 只返回给主编排，不由 executor 执行
   - `run_already_created: true` → 直接执行 exact Run；严禁把 birth packet 元数据当作 skill prompt。**非首步而 brief 缺 Previous step / Upstream 时返回 BLOCKED，不静默继续**（缺前序上下文说明 handoff 未落 run.json，属编排链断裂）
...
- Ordinary Knowhow creation requires only `type`, `title`, and `content`; ordinary Spec creation requires only `category`, `title`, and `content`. Treat every other content field as optional advanced metadata.
- Repository context is host-owned. Omit `targetRepoId` for current-repository writes. Pass `targetRepoId` only when the Run explicitly selects a linked physical repository and the host birth packet supplies both that exact stable UUID and a live write capability for the matching corpus (`spec` or `knowhow`).
- Never infer an ID from cwd, repository name, alias, or path; never persist alias/path as repository identity. Missing or stale linked authority is `BLOCKED`, not a reason to guess.
- Audit is always read-only; `--prune` only reports deterministic suggestions and audit has no `--apply`, mutation, delete, or purge path. Normalization is the only report-fenced migration apply: generate and review `maestro knowledge normalize --report <path>` before a separate same-path `--apply`; never bulk-rewrite as a finish-work side effect.
...
|-------|----------|-------------|
| `session_id` | Yes | exact canonical Session ID from the birth packet |
| `run_id` | Yes | exact allocated Run ID; missing means BLOCKED |
| `task` | Yes | resolved command, positional args, goal, and input Artifact refs |
| `continuation` | Yes | structured contract returned unchanged to the orchestrator |
...
- 收到 session_id 即开始执行
- dispatch prompt 必须携带 exact `session_id` + `run_id` birth packet；一切执行上下文（上游产物、前序 handoff、goal、refs、positional args）从 resolved `task`/birth packet 或 `run brief`（回溯）获取。仅有 `session_id` 时返回 BLOCKED，不猜 mutation authority
