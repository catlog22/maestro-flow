...
4. **失败 worker 必须收尾**：任一 worker 失败/超时/返回错误时，必须等齐（或停掉）其余仍在跑的 worker 再返回，禁止首个失败就丢下其他子 Agent；executor 结束前其作用域内不允许仍有运行中的子 Agent
5. **返回**：将最终执行输出作为文本返回（主流程通过 task-notification 接收）
