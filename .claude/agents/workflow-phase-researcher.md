...
When the phase requires an architecture or system-pattern decision, follow the
shared Architecture Template References rule. Reuse upstream evidence or, when
absent, search/load once and preserve it in `## Architecture Template Evidence`.
Do not search for implementation-only phases.
...
- `architecture_template_evidence` (optional) — upstream reference-only template evidence to preserve
