...
10. **Check architecture-template evidence**:
    - no template search/load is performed by the checker;
    - `shared_context.architecture_template_search` holds only envelope status/query and `shared_context.architecture_templates` is an array of template records;
    - every task template ID exists in `shared_context.architecture_templates`;
    - source=arch-kb, kind=template, reference_only=true;
    - adopted/adapted entries have rationale and applies_to;
    - non-loaded status, candidate, or rejected entries cannot produce task requirements;
    - template IDs are absent from project-knowledge consumption fields.
