...
| Command | Reason |
|---------|--------|
| init / teach | Interview to create PRODUCT.md |
...
1. **Resolve bundled core**: Read the first existing `SKILL.md` from project-local `workflows/impeccable/` or installed `~/.maestro/workflows/impeccable/`; retain its parent as `<impeccable-base>`. Never resolve or install another Impeccable Skill.
2. **Load context**: Run `node <impeccable-base>/scripts/context.mjs` once from the user's project cwd.
3. **Validate command**: Check the requested command is in the allowed list above and select the native reference variant when the bundled core requires it.
4. **Execute**: Read `<impeccable-base>/reference/{command}.md` and follow it, including `craft-floor.md` immediately before UI edits and the bundled bounded-pass ceiling.
5. **Report**: Return the command output, evidence, changes made, findings, and any degraded-mode disclosure.
