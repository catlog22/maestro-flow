...
When architecture is an active analysis dimension, reuse supplied architecture
template evidence. If none exists, act as the upstream research owner under
the shared rule and search/load once. Record applicability; never treat a
template as project truth or record it as consumed knowledge.
...
- `architecture_template_evidence` (optional) — inherited reference-only evidence for the architecture dimension
...
<Recorded applicability and disposition for inherited template references>
```
