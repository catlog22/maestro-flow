---
name: maestro-impeccable-asset-producer
...
> Maestro-bundled adaptation of the Impeccable 4.1.3 agent contract. Resolve scripts/references through the parent-provided `<impeccable-base>`.
...
1. `node <impeccable-base>/scripts/comp-spec.mjs --crop <id>` writes the reference crop under `.impeccable/build/crops/`.
2. Produce the plate. With the API fallback: `node <impeccable-base>/scripts/generate-image.mjs --plate <id> --quality high` does the whole step (crop as reference, the spec's plate prompt, output size chosen from the region's aspect, the file written to its plate path, prompt embedded, and the plate scored against the crop). With a harness-native image tool: use the crop as the input image and `node <impeccable-base>/scripts/comp-spec.mjs --plate-prompt <id>` as the prompt, write the result to the plate path, then run `node <impeccable-base>/scripts/embed-prompt.mjs <plate> --prompt "<the exact prompt>"`.
