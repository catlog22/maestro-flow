...
<run_mode>
**Session mode:** `run`. The coordinator MUST call `maestro run create team-interactive-craft -- $ARGUMENTS` before creating workers and retain the returned `run_id`/`run_dir`.
