...
Each spec file is the primary document for a category. `spec load --category` loads the primary file in full, plus cross-file keyword-matched entries.
...
| `sid` | No | Stable identity (format `S-YYYYMMDD-xxxx`, auto-generated by `spec add`) |
...
Tools are knowhow documents tagged with `tool: true` in YAML frontmatter. `spec load --category` auto-scans `knowhow/` for entries matching category + tool, appending summaries.
