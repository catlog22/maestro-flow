...
Tools need no dedicated register/execute command: capture a `tool: true` knowhow document via `/maestro-knowhow` (recipe type + `--tool`), and agents auto-discover and inject its summary through `maestro spec load --category`.
...
```
maestro-init -> spec init       analyze -> plan, implement
plan         -> implement, test execute -> implement, analyze
