...
| 工作流入口命令（第一档 step，§7.6） | ~15 个独立 Skill 入口（核心链 + quality + 上游） | **2**（next + ralph）；第二/三档入口保留 | §七 |
...
### 3.9a 内容三层拆分与 prep 字段

**现有入口命令与 workflow 文档的综合拆分**——原入口命令由若干标签组成（frontmatter + `<purpose>` + `<invariants>` + `<required_reading>` + `<process>`），按"谁在什么时刻需要"拆为三层：

| 现有组成 | 层 | 交付时机 | 去处 |
|------|------|------|------|
| frontmatter contract | **结构层**（YAML） | prepare 预读 | `steps/{name}.md` frontmatter |
| `<purpose>` | 结构层 | prepare 预读 | frontmatter `purpose:` 一句话 |
| `<invariants>` | 结构层 | prepare 预读 + create 回显 | frontmatter `invariants: []` |
| `<process>` 执行正文 | **执行层**（md） | create 全量返回 | `steps/{name}.md` 正文 |
| `<required_reading>` 中协议文档（run-mode.md） | **消亡** | — | 协议行为由 prepare/create/brief 返回包直接体现——"零协议学习"的完成形态，不再靠读文档学协议 |
| `<required_reading>` 中领域 workflow 文档（brainstorm.md / blueprint.md…） | **参考层** | create 只返回 `{path, 摘要, 何时读}` 清单，**deferred Read** | 原位保留，或迁 `steps/ref/` |

分层依据：结构层是思考材料（prepare 时刻），执行层是干活指引（create 时刻、需全量在场），参考层是备查细节（执行中按需、全量嵌入即浪费）。
...
reads: ["steps/ref/brainstorm.md"]         # 可选 → 从 prepare 返回的参考清单中挑选本次深读项
```
...
| 2 | **正文单源 + 三层交付**：step `.md` 是唯一内容源，CLI 按 §3.9a 分层交付——结构层 YAML 给 prepare，执行层正文 create 全量返回，参考层只给 deferred 清单（LLM 按需 Read）；协议文档（run-mode.md）整体消亡 | 出现第二份存储即产生内容漂移，违反命令自包含约束；全量嵌入参考文档是上下文浪费 |
| 3 | **prepare 可选**：不带 `--prep` 的 create 退化为现行为（仅 contract 派生门，`goal: null`） | 轻量场景（companion 类 sidecar、`ralph --preset quick`）免三步开销；与"无 contract 也能跑"同构的渐进增强 |
...
| `maestro-ralph*` / `companion` / `roadmap` | orchestration 子键（`inserted_by`/`decision_ref` 等） | 按 §3.2 新 chain/decision_points 键名对齐 | 每文件 3–8 行 |
...
| 层 | 组成 | 职责 |
|---|------|------|
| **入口层** | `next`（单步·人在环）+ `ralph`（循环·自治） | 意图解析、路由、run 生命周期驱动——**不含任何领域内容** |
...
```text
~/.maestro/                      # 全局注册表
├── steps/{name}.md              # 步骤（结构化 frontmatter + 执行正文，§3.9 三层拆分）
...
**step 单形态 `{name}.md`**：YAML frontmatter 承载结构层（供 prepare 预读），md 正文承载执行层（create 返回）——拆分规则见 §3.9。正文不塞进 YAML 块标量，散文是给 LLM 读的，md 是正确介质。
...
| 档 | 形态 | 判定标准 | 归属 |
|---|------|------|------|
| **Step** | `steps/{name}.md`，无独立入口，next/ralph 驱动 | 参与核心产物链：contract 的 consumes/produces 与其他 step 交接 | analyze / plan / execute / verify / review / test / debug / auto-test / retrospective / collab / grill / brainstorm / blueprint / roadmap |
...
- 例外照旧：`maestro-init`（Session 前例外，指南 §十三）、`maestro-companion`（sidecar）、`session-seal` / `fork` / `merge`（session 动词，ralph 可在 seal 决策点自动触发）。
