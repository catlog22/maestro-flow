...
- ralph 的自动门只有两类：**质量门**（`post-execute`/`post-review`/`post-test`——代码能跑吗）和**目标门**（`post-goal-audit`——`done_when` 达成吗），见 `maestro-ralph.md:164-185`。
...
**R4 `-y` 横切**
- `maestro-ralph.md:352,354,364,126`（broad/phase 硬护栏、medium/narrow 派生）、`:164-185`（仅质量/目标门，无 scope 门）、`:501-535`（goal-audit）
...
- *（对抗层已修正 ANT-2-2 两处过度声称：所谓 "maestro.md:290 与决策节点矛盾" 是误读——该行正确限定于 `source:"maestro"` 静态链；"verify 回退到 singles/quick" 机制有误，实为图加载期失败。三引擎结论本身独立成立。）*
