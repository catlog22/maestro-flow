...
| `preflight-guard` | PreToolUse | Bash\|Write\|Edit\|Agent | standard | — | 命令执行前预检守卫，验证操作合法性 |
| `spec-validator` | PreToolUse | Write\|Edit | standard | — | 规范写入验证，确保内容符合 entry 格式 |
| `keyword-spec-injector` | UserPromptSubmit | — | standard | — | 从用户输入提取关键词，匹配注入相关规范条目 |
...
|------|---------|---------|
| `none` | 无 Hook | 完全手动控制 |
| `minimal` | Statusline + spec-injector | 日常开发，轻量监控 + 自动规范注入 |
...
maestro hooks uninstall --global          # 卸载全局 Hook
maestro hooks uninstall --project         # 卸载项目级 Hook

# 查看状态
maestro hooks status                       # 显示所有 Hook 安装状态和级别
maestro hooks list                         # 列出可用 Hook 及定义
maestro hooks config                       # 显示当前 hook 配置
