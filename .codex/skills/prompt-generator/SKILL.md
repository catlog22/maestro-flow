...
3. **`<process>`** — numbered steps (GSD workflow style):
   ...
   - Each step: banner display, validation, agent spawning via `spawn_agent()`, error handling
...
- Agent spawning uses `spawn_agent({ task_name: "<task_name>", message: "<message>" })` pattern
...
3. **`<process>`** — numbered steps (GSD workflow style):
   ...
   - Each step: validation, agent spawning via `spawn_agent()`, error handling
...
| Agent spawning (if complex) | `spawn_agent({` call with `agent_type` (V2 field, not `subagent_type`) |
...
| Content separation | No `request_user_input`, no banner display, no argument parsing |
