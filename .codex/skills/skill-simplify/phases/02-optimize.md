...
Run `ccw maestro-spec load --category validation` for verification rules and acceptance criteria to validate optimization preserves functional integrity.
...
| Redundant "Next Phase" prose | Remove when update_plan handles flow |
...
| Target Pattern | Action |
|----------------|--------|
| Multiple similar request_user_input blocks | Extract shared function with mode parameter |
...
| update_plan full blocks x N | Template ONCE, subsequent as one-line comment |
