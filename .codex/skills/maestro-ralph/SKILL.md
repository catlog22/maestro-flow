...
<purpose>
Adaptive lifecycle orchestrator: locate step → resolve args → load context → dispatch spawn_agent(ralph-executor) per step (agent 调 `ralph next` + 执行) → extract signals → drift check → ralph complete → evaluate decision → next step → loop.
...
4. **Unified unnamed dispatch** — 执行 Agent 和评估 Agent 均使用 unnamed spawn_agent()，结果通过 task-notification `<result>` 回传：
   - **执行 Agent**（A_STEP_DISPATCH）：`spawn_agent()` 不传 name — executor 内部编排也用 unnamed Agent（子结果自动回流 executor，嵌套套娃模型）
   - **评估 Agent**（A_AGENT_EVALUATE / A_AGENT_GOAL_AUDIT / A_AGENT_REGROUND）：同样 `spawn_agent()` 不传 name
...
12. **step command 在 A_BUILD_STEPS 解析** — 通过 `maestro ralph skills --platform codex --json --quiet` 预校验
...
> **执行模型**：每个 step 由 spawn_agent(ralph-executor) 派发执行，非主会话内联。Agent 内部调 `maestro ralph next` 获取 skill prompt 并执行，结果通过 task-notification 回传主流程。
...
9. **command_path 解析**（每个执行 step，decision 节点跳过）：
   ...
   - **预校验通过 `Bash("maestro ralph skills --platform codex --json --quiet")`** 一次性拉取 claude 平台可用 commands + skills（global + project，project 覆盖 global），匹配 skill 名得到：
...
7. spawn_agent() 返回 agentId → 等待 task-notification（status=completed 时 `<result>` 含 executor 输出）
...
- [ ] One agent per step — `spawn_agent({ task_name: "<task_name>", message: "<message>" })` 每步派发一个 unnamed executor
