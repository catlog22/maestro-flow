...
<next_step_routing>
- Deep feasibility → step `analyze` (`maestro run prepare --platform codex analyze` + `maestro run create analyze --session YYYYMMDD-analyze-{topic} --intent "{topic}"`)
- Plan from conclusions → step `plan` (`maestro run prepare --platform codex plan` + `maestro run create plan --session YYYYMMDD-plan-{topic} --intent "{goal}" -- --dir {dir}`)
- Expand → step `brainstorm` (`maestro run prepare --platform codex brainstorm` + `maestro run create brainstorm --session YYYYMMDD-brainstorm-{topic} --intent "{topic}"`)
</next_step_routing>
