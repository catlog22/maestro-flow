...
description: Detect and realign .workflow/ artifact drift against code reality
  after refactoring
argument-hint: '[-y|--yes] [-c|--concurrency 4] [--continue] "--scope
  <all|roadmap|spec|codebase|state|issue|knowhow|project> [--since
  YYYY-MM-DD|commit|HEAD~N] [--depth shallow|deep] [--dry-run] [--report]
  [--auto-archive]"'
...
1. Before domain work, call `maestro run create manage-drift-realign -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
