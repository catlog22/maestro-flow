...
- Reconcile session state with list_agents on startup
...
### Step 3: Reconcile with list_agents

Compare list_agents() with session.completed_tasks, bidirectional sync.
...
For each task in task-analysis, check if exists in list_agents, create if missing.
