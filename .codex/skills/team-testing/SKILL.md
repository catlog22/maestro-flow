...
argument-hint: "[scope] [-y|--yes] [-c|--concurrency N] [--continue] [--pipeline
  targeted|standard|comprehensive]"
...
1. Before domain work, call `maestro run create team-testing -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
