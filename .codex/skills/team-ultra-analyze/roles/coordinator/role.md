...
5. Write team-session.json with mode, requirement, timestamp
...
3. For each member in pending_shutdown, send shutdown request:
   ```javascript
   followup_task({ target: "<member-name>",
...
**Note**: Workers that completed Phase 5-F and reached STOP may have already terminated. send_message to a terminated agent is silently ignored — this is safe. Only resident agents (e.g., supervisor) require explicit shutdown.
