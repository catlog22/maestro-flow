...
<purpose>
Validate that command delegation prompts (spawn_agent() calls) and agent role definitions respect GSD content separation boundaries. Detects 7 conflict dimensions: role re-definition, domain expertise leaking into prompts, quality gate duplication, output format conflicts, process override, scope authority conflicts, and missing contracts.
...
```bash
# Search both spawn_agent() (current) and Task() (legacy GSD) patterns
grep -n "spawn_agent(\|Task(" "$COMMAND_FILE"
grep -n "subagent_type" "$COMMAND_FILE"
```

For each `spawn_agent()` call, extract:
...
For each spawn_agent() call, extract structured blocks from the prompt content:
...
- [ ] All spawn_agent() calls extracted from commands with full prompt content
