...
argument-hint: <template-slug|path> [--context key=value...] [-c [session-id]]
  [--list] [--dry-run]
...
1. Before domain work, call `maestro run create maestro-player -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
