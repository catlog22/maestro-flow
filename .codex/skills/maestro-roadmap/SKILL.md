...
argument-hint: '"<requirements>" [-m progressive|direct|auto] [-y|--yes] [-c]
  [--phases N] [--skip-research] [--from <source>] [--from-brainstorm
...
1. Before domain work, call `maestro run create maestro-roadmap -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
