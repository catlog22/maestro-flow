...
2. **Route** (mode-tuned target for `bug` — see mode's **discover routing** note):
   ...
   Normal: request_user_input per hit | `-y`: auto-fix bugs with fix_template, create issue for rest
...
3. **Goal audit (hardened):**
   ...
   - **Hard rule:** generalize + discover goals CANNOT be `skipped` unless `skip_generalize == true`. Fix/verify goals CANNOT be `skipped` unless `skip_fix == true`. Pending without flag → `failed` (Normal: request_user_input | `-y`: record `failed`)
...
|---------------|--------|------|
| Mode ambiguous at dispatch | request_user_input (5-way) | E000 |
| Fix/improvement confirmation | request_user_input | auto-proceed, `deferred` |
| A_DISCOVER hit routing | request_user_input | auto-fix bugs w/ template, issue for rest |
| A_RECORD pending decisions | request_user_input | skip (show deferred count) |
| Goal audit unflagged-pending | request_user_input | record `failed` |
...
Pending decisions must request_user_input — no silent resolve.
```
...
**A_DIAGNOSE** — (1) Hypotheses from evidence, ranked [HIGH]/[MEDIUM]/[LOW] → §4. (2) Test each → evidence phase=diagnosis. (3) Ambiguity → evidence phase=decision; Normal: request_user_input | `-y`: defer. (4) Confirmed → `session.json.root_cause` + §5. Mark G1.

**A_ESCALATE_DIAGNOSIS** — `diagnosis_retries++`. < 3: `maestro delegate --role analyze`, new hypotheses, → S_DIAGNOSE. >= 3: Normal → request_user_input | `-y` → INCONCLUSIVE → S_RECORD.

**A_FIX** — (1) Present root cause + proposed fix. Normal: request_user_input | `-y`: auto proceed. (2) Implement fix, evidence phase=decision.
...
**A_DIAGNOSE** — Root cause analysis for critical/high findings — don't fix symptoms. (1) Group by dimension, prioritize by severity; for each: hypothesis → trace code path + git history → evidence phase=diagnosis. (2) Ambiguity → evidence phase=decision; Normal: request_user_input | `-y`: defer. (3) CLI-assisted for complex findings (`run_in_background: true`). (4) Write `session.json.diagnoses[]`. Update §4. Mark G3.
...
```
Normal: request_user_input per tier | `-y`: auto-fix all. Remaining > 0 → retry (max_fix_rounds = 5). Unchanged 2 rounds → classify each individually. After 5 rounds remaining > 0 → escalate: Normal: request_user_input (continue/accept/reclassify) | `-y`: classify remaining as `deferred`, proceed. Blanket "pre-existing" forbidden. Commit per tier: `"odyssey-review({slug}): FIX-{tier} — {N} items fixed"`.
...
**A_DIVERGE** — Goes beyond defect fixing — "what would make this delightful?" **Step 1 — 2 parallel Agents:** Polish spawn_agent(shadows, borders, transitions, hover, feedback, empty states, skeleton loading, scroll behavior) + Delight spawn_agent(motion design, progressive disclosure, smart defaults, contextual hints, celebratory feedback, personality in copy). Each returns `[{idea, category (polish|delight), impact, effort, description, inspiration}]`. **Step 2 — CLI-assisted:** `maestro delegate --role analyze --mode analysis` — polish opportunities, micro-interactions, visual rhythm, delight moments. **Step 3 — Consolidate:** merge audit findings + divergent ideas → prioritized list (severity × impact × effort). Evidence phase=diverge. Update §4. Mark G3.

**A_FIX** — Skip if `--skip-fix`. (1) Exhaustive fix: ALL findings/ideas by priority tier (critical → high → medium → low + high-impact ideas). After each tier, re-review — new findings append. (2) Each fix → evidence phase=fix. (3) Normal: request_user_input per-fix | `-y`: auto-proceed, record `deferred`.
