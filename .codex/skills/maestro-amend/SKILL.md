...
argument-hint: "[description] [--from-verify <dir>] [--from-review <dir>]
  [--from-session <id>] [--from-issues ISS-xxx,...] [--scan] [--dry-run] [-y]"
...
1. Before domain work, call `maestro run create maestro-amend -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
