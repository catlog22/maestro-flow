...
description: Use when designing, auditing, polishing, or improving frontend UI —
  websites, dashboards, landing pages, components
...
1. Before domain work, call `maestro run create maestro-impeccable -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
