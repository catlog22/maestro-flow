...
| update_plan Pattern | Attachment/collapse examples | SKILL.md update_plan Pattern |
...
const basicInfo = request_user_input({
  questions: [
    {
...
for (let i = 0; i < phaseCount; i++) {
  const phaseInfo = request_user_input({
    questions: [
      {
...
const conditions = request_user_input({
  questions: [{
...
const workflowConfig = {
  ...
  allowedTools: ["Agent", "request_user_input", "update_plan", "Read", "Write", "Edit", "Bash", "Glob", "Grep", "Skill"],
  ...
  // Features
  features: {
    hasAutoMode: true,                  // Interactive preference collection (request_user_input)
...
const confirm = request_user_input({
  questions: [{
