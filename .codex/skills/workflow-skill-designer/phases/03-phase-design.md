...
|-------------------|-----------|-----------------|
| Flag parsing (`$ARGUMENTS.includes(...)`) | Grep: `\$ARGUMENTS\.includes` | SKILL.md via request_user_input → `workflowPreferences` |
...
| Skill routing for inter-phase (`spawn_agent({ task_name: "...", message: "Execute skill ..." })`) | Grep: `Skill\(skill=` | Direct `Read("phases/0N-xxx.md")` |
...
function extractPhaseFromCommand(phase, config) {
  ...
  if (!bodyContent.includes('## Output')) {
    ...
    phaseContent += `- **update_plan**: Mark Phase ${phase.number} completed, Phase ${phase.number + 1} in_progress\n`;
  }
...
function verifyContentPreservation(sourceContent, phaseContent) {
  const checks = {
    ...
    // Count request_user_input calls
    sourceAUQ: (sourceContent.match(/request_user_input/g) || []).length,
    phaseAUQ: (phaseContent.match(/request_user_input/g) || []).length,
  ...
  if (checks.phaseAUQ < checks.sourceAUQ) {
    issues.push(`Missing request_user_input: source=${checks.sourceAUQ}, phase=${checks.phaseAUQ}`);
  }
...
Some commands mix orchestrator-level instructions (coordination, update_plan) with execution detail. Separation rules:

| Content in Source Command | Goes To | Rule |
|---------------------------|---------|------|
| Phase execution steps, agent prompts, bash commands | **Phase file** | Preserve verbatim |
| update_plan update examples specific to this phase | **Phase file** (optional) | Keep if useful for context |
...
function generatePhaseFromRequirements(phase, config) {
  ...
  phaseContent += `- **update_plan**: Mark Phase ${phase.number} completed\n\n`;
...
- **update_plan**: Mark Phase 3 completed, Phase 4 in_progress
