...
function validateSkillMdSections(config) {
  ...
  const recommendedSections = [
    ...
    { name: 'update_plan Pattern', pattern: /## update_plan Pattern/ },
...
Design Patterns Applied:
  ...
  ${config.features.hasTodoWriteSubTasks ? '✓' : '○'} update_plan attachment/collapse
  ${config.features.hasConditionalPhases ? '✓' : '○'} Conditional phase execution
  ${config.features.hasAutoMode ? '✓' : '○'} Interactive preference collection (request_user_input)
...
- **update_plan**: Mark Phase 4 completed (all tasks done)
