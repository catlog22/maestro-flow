...
Generate the SKILL.md orchestrator file from workflowConfig, applying all coordination patterns (progressive loading, update_plan, data flow, conditional execution).
...
- Build update_plan attachment/collapse patterns from phase definitions
...
function convertCommandFrontmatter(commandFm, config) {
  return {
    ...
    // "Skill(*), update_plan(*), Read(*)" → "Task, request_user_input, update_plan, Read, Write, Edit, Bash, Glob, Grep, Skill"
  };
}
...
function expandToolWildcards(toolsStr) {
  const expanded = toolsStr
    ...
    .replace(/update_plan\(\*\)/g, 'update_plan')
    .replace(/Read\(\*\)/g, 'Read')
    .replace(/Bash\(\*\)/g, 'Bash')
    .replace(/Glob\(\*\)/g, 'Glob')
    .replace(/Grep\(\*\)/g, 'Grep')
  ...
  const baseTools = ['Task', 'request_user_input', 'update_plan', 'Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep'];
...
## Step 2.6: Generate update_plan Pattern

```javascript
function generateTodoWritePattern(config) {
  let section = '## update_plan Pattern\n\n';
  ...
  section += '   - Sub-tasks are **attached** to orchestrator\'s update_plan\n';
  ...
  // Generate update_plan examples for phases with sub-tasks
...
When the skill has configurable behaviors (auto mode, force options, etc.), generate the request_user_input-based preference collection section for SKILL.md:
...
function generateInteractivePreferenceCollection(config) {
  ...
  section += 'Collect workflow preferences via request_user_input before dispatching to phases:\n\n';
  section += '```javascript\n';
  section += 'const prefResponse = request_user_input({\n';
  section += '  questions: [\n';
...
- **update_plan**: Mark Phase 2 completed, Phase 3 in_progress
