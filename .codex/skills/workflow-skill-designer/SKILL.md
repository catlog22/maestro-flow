...
### Pattern 2: update_plan Attachment/Collapse
...
Workflow preferences (auto mode, force explore, etc.) MUST be collected via request_user_input in SKILL.md **before** dispatching to phases. Phases reference these as `workflowPreferences.{key}` context variables.
...
|------------|--------|------------------|
| Flag parsing (`$ARGUMENTS.includes(...)`) | Preferences collected in SKILL.md | SKILL.md via request_user_input |
...
| Skill routing for inter-phase (`spawn_agent({ task_name: "...", message: "Execute skill ..." })`) | Use direct phase read | Direct `Read("phases/...")` |
...
|-------|------|-----------|
| **1. Active Phase Protection** | 预防 | update_plan `in_progress` 标记当前 phase → compact 时保留该 phase 完整内容，不压缩 |
...
#### Layer 1: Active Phase Protection (update_plan 联动)
...
Phase 1: Requirements Analysis    ← update_plan tracks status
   └─ Ref: phases/01-xxx.md
Phase 2: Execution Engine         ← update_plan tracks status
...
|-------|----------|---------|---------|
| 1 | phases/01-xxx.md | Planning | update_plan 驱动 |
| 2 | phases/02-xxx.md | Execution | update_plan 驱动 + 🔄 sentinel |
...
> 1. This phase is update_plan `in_progress` (active phase protection)
...
```javascript
// Phase N is tracked by update_plan — active phase protection applies.
...
| update_plan Pattern | Attachment/collapse lifecycle | Command update_plan sections |
...
{request_user_input code with preference derivation → workflowPreferences}
...
| Phase | Document | Purpose | Compact |
|-------|----------|---------|---------|
| 1 | [phases/01-xxx.md](phases/01-xxx.md) | ... | update_plan 驱动 |
| N | [phases/0N-xxx.md](phases/0N-xxx.md) | ... | update_plan 驱动 + 🔄 sentinel |
...
...
| What's the update_plan granularity? | update_plan Pattern | Some phases have sub-tasks, others are atomic |
...
| Does it need preference collection? | Interactive Preference Collection | Collect via request_user_input in SKILL.md, pass as workflowPreferences |
