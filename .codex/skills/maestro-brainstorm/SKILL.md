...
description: Use when exploring ideas, evaluating approaches, or needing
  multi-perspective analysis before implementation
argument-hint: "[topic] [-y|--yes] [--concurrency N] [-c|--continue] [--count N]
  [--skip-questions] [--review-only]"
...
1. Before domain work, call `maestro run create maestro-brainstorm -- $ARGUMENTS` and retain the returned `run_id`, `run_dir`, and `upstream`.
