...
| `odyssey --mode debug` | 假设驱动调试 | 根因是什么？ | `DBG-{NNN}` |
| `quality-refactor` | 反思驱动重构 | 技术债是否收敛？ | `WBR-{NNN}` |
| `manage sync codebase` | 文档同步 | 文档与代码是否一致？ | — |
...
| 仍有问题 | `/odyssey --mode debug "<from-uat {phase}>"` |
...
| 发现 Bug | `/odyssey --mode debug "<from-uat {phase}>"` |
| 最大迭代，>80% | `/maestro "<test intent: {phase}>"` |
| 最大迭代，<80% | `/odyssey --mode debug "{phase}"` |

---

## odyssey --mode debug — 假设驱动调试
...
代码刚执行完
  ...
  │    └─ 有问题 ──> /odyssey --mode debug "<from-uat <phase>>"
  │
  ├─ 需要自动化测试？──> /maestro-ralph --engine swarm "<phase>"
  │    ├─ 收敛 ──> 已通过 maestro-ralph 决策门控
  │    └─ 发现 Bug ──> /odyssey --mode debug "<from-uat <phase>>"
  │
  ├─ 有已知 Bug？──> /odyssey --mode debug "<issue>"
  │    ├─ 根因明确 ──> /maestro-next "<phase> --gaps"
  │    └─ 不确定 ──> 继续调试
  │
  ├─ 需要减少技术债？──> /quality-refactor <scope>
  │    ├─ 测试通过 ──> /manage sync codebase
  │    └─ 测试失败 ──> /odyssey --mode debug "<scope>"
  │
  ├─ 代码改了文档没更新？──> /manage sync codebase
  │
...
| `odyssey --mode debug` 确认根因 | `/maestro-next "{phase} --gaps"` |
