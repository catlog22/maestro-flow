...
maestro knowledge audit --scope all --json              # 只读 compatibility/identity/link/promotion 诊断
...
`store_knowhow` 是 MCP 内置工具。ordinary Knowhow 只要求 type/title/content，ordinary Spec 只要求 category/title/content；九种 Knowhow 类型保持兼容：
...
条目自动由 WikiIndexer 索引（type=knowhow）。当前仓库 MCP 写省略 `targetRepoId`；仅 host 为显式 linked write 提供 exact UUID + live capability 时 Agent 才传。CLI `--repo` 选择物理目标，repeatable `--applies-to-repo` 只记录适用范围；它们与 host/MCP 的 exact-UUID `targetRepoId` 不同。禁止从 cwd/name/alias/path 推导或持久化 identity。Legacy aliases 仅读兼容，audit 不改写；normalize 必须 report-first。
