...
### 2.1 maestro-learn consult — 统一复盘
...
```bash
/maestro-learn consult                                    # 默认：两种 lens 全量分析最近 7 天
/maestro-learn consult --lens git --days 14               # 仅 Git 分析，最近 14 天
/maestro-learn consult --lens decision --phase 2          # 仅决策分析，聚焦 Phase 2
/maestro-learn consult --lens all --author alice --compare # 全量分析，按作者过滤，对比上次复盘
```
</details>
...
| 你想做什么 | 使用命令 | 示例 |
|-----------|---------|------|
| 回顾过去一周的工作质量 | `maestro-learn consult` | `--lens git --days 7` |
| 检查架构决策是否仍然有效 | `maestro-learn consult` | `--lens decision --phase 2` |
...
| 场景 | 步骤 |
|------|------|
| **新成员 Onboarding** | `maestro-learn follow src/` → `maestro-learn decompose src/core/ --save-wiki` → `maestro-learn consult --lens git --days 30` |
...
| **迭代复盘** | `maestro-learn consult --lens all --days 14 --compare` → `maestro-learn investigate "高 churn 原因"` → `maestro-learn decompose --save-spec` |
