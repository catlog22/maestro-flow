...
Maestro 学习工具集的完整使用手册，涵盖 5 个 `/learn` 子命令的原理、用法和协作模式。
...
### 与 /manage knowledge capture 的区别

| 维度 | /learn 子命令 | /manage knowledge capture |
|------|---------------|--------------|
...
简单规则：**需要思考用 /learn，需要记录用 /manage knowledge capture**。

---

## 二、命令详解

### 2.1 learn consult — 统一复盘
...
```bash
/learn consult                                    # 默认：两种 lens 全量分析最近 7 天
/learn consult --lens git --days 14               # 仅 Git 分析，最近 14 天
/learn consult --lens decision --phase 2          # 仅决策分析，聚焦 Phase 2
/learn consult --lens all --author alice --compare # 全量分析，按作者过滤，对比上次复盘
```
</details>
...
```bash
/learn consult src/auth/jwt.ts                    # 默认 review 模式
/learn consult src/core/ --mode challenge          # 对抗式质疑
/learn consult HEAD --mode consult                 # 交互式 Q&A
/learn consult 2 --mode review                     # 审查 Phase 2 的计划
```
</details>
...
| 你想做什么 | 使用命令 | 示例 |
|-----------|---------|------|
| 回顾过去一周的工作质量 | `learn consult` | `--lens git --days 7` |
| 检查架构决策是否仍然有效 | `learn consult` | `--lens decision --phase 2` |
...
| 审查代码质量（多视角） | `learn consult` | `src/api/` |
| 对方案进行压力测试 | `learn consult` | `HEAD --mode challenge` |
| 就某个实现向 AI 请教 | `learn consult` | `plan.json --mode consult` |
...
| 场景 | 步骤 |
|------|------|
| **新成员 Onboarding** | `learn follow src/` → `learn decompose src/core/ --save-wiki` → `learn consult --lens git --days 30` |
| **架构决策前** | `learn follow src/auth/ --depth deep` → `learn consult --mode review` → `learn consult --mode challenge` → `learn investigate "影响范围"` |
| **迭代复盘** | `learn consult --lens all --days 14 --compare` → `learn investigate "高 churn 原因"` → `learn decompose --save-spec` |
| **问题排查（理解而非修复）** | `learn investigate "延迟原因"` → `learn follow 关键文件` → `learn consult --mode consult` |

### 命令间的自然衔接

```
learn follow → learn decompose      # 从理解到模式提取
learn follow → learn consult        # 从理解到多视角验证
learn decompose → /spec add         # 从模式发现到规范录入
learn consult → learn investigate   # 从复盘发现到深入探究
learn investigate → learn follow    # 从问题定位到深入阅读
learn consult → learn decompose     # 从质疑到系统化拆解
```
