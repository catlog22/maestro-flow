...
知识回收将执行产物中的碎片提取、分类路由、写入持久存储，再由下游命令消费后反哺新一轮执行，形成完整的知识闭环。三个阶段：**Extract**（`/manage knowledge harvest` 提取）→ **Route**（分类引擎自动路由）→ **Persist**（写入 wiki/spec/issue）。

### 三大知识存储

| 存储 | 路径 | 存什么 | 谁消费 |
|------|------|--------|--------|
| **Wiki** | `.workflow/wiki/` | 观察发现、通用洞察、知识图谱 | `/manage knowledge wiki connect`、`/manage knowledge wiki digest` |
| **Spec** | `.workflow/specs/` | 编码规范、架构决策、模式规则 | `/spec load`、Hook 自动注入 |
| **Issue** | `.workflow/issues/issues.jsonl` | 未解决的 bug、风险、待办 | `/manage issue`、`/maestro-ralph --engine swarm --script wf-analyze` |

### 与 knowhow 的关系

Harvest 提取的碎片路由到 wiki/spec/issue。Knowhow（`.workflow/knowhow/`）是独立的完整知识文档系统，由 `/manage knowledge capture` 主动创建，二者互补：**Harvest** = 被动回收，**Knowhow** = 主动捕获。

---

## 二、manage knowledge harvest 详解

### 命令语法

```bash
/manage knowledge harvest                                      # 扫描所有产物，交互选择
/manage knowledge harvest <session-id>                         # 回收指定会话
/manage knowledge harvest <path>                               # 回收指定目录
/manage knowledge harvest --recent 7                           # 只看最近 7 天
/manage knowledge harvest --source analysis                    # 只回收分析产物
/manage knowledge harvest <target> --to wiki                   # 强制全部路由到 wiki
/manage knowledge harvest <target> --dry-run                   # 预览，不写入
```
...
## 三、manage knowledge knowhow 详解

### 命令语法

```bash
/manage knowledge knowhow                                  # 列出全部（默认）
/manage knowledge knowhow search "认证流程"                  # 全文搜索
/manage knowledge knowhow view KNW-20260510-1430           # 查看指定条目
/manage knowledge knowhow edit MEMORY.md                   # 编辑系统记忆
/manage knowledge knowhow delete TIP-20260510-0900         # 删除（需确认）
/manage knowledge knowhow prune --tag deprecated --before 2026-04-01  # 批量清理
```
...
## 四、manage knowledge capture 详解

### 命令语法

```bash
/manage knowledge capture compact "认证模块开发进度"       # 会话压缩
/manage knowledge capture template                       # 交互式录入模板
/manage knowledge capture recipe "部署流程"                # 操作配方
/manage knowledge capture reference --source https://...  # 外部文档摘要
/manage knowledge capture decision                       # 架构决策记录
/manage knowledge capture tip "TypeScript 泛型推断陷阱"    # 快速提示
/manage knowledge capture                                # 交互选择（9 种类型）
```
...
```
执行过程 → /manage knowledge capture → .workflow/knowhow/ → wiki-index.json → 检索复用
```
...
| 命令 | 产出 | 路由到 |
|------|------|--------|
| `/learn consult` | git 活动回顾、决策回顾 | `specs/learnings.md`（`<spec-entry>`） |
| `/learn decompose` | 任务分解经验 | knowhow（recipe） |
| `/learn investigate` | 调查过程记录 | knowhow（reference / tip） |
| `/learn follow` | 跟进学习记录 | knowhow（reference） |
| `/learn consult` | 多视角分析结果 | wiki / spec |

### 推荐工作流

| 场景 | 步骤 |
|------|------|
| **日常开发** | `/maestro-ralph continue` → 完成后随手记 → `/manage knowledge capture tip "发现的技巧"` |
| **里程碑结束** | `/manage knowledge harvest --recent 30` → `/manage knowledge capture compact` → `/manage knowledge wiki connect --fix` |
| **项目交接** | `/manage knowledge knowhow list` → `/manage knowledge knowhow search "核心概念"` → `/spec load --role implement` |
