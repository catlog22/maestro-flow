...
**run / session** -- 管理 topic Session、Run 生命周期和命令链：
...
| 选项 | 说明 |
|------|------|
| `run start --cmd` | 人类入口：创建单次 Run；命令输入用 `--arg` 传入 |
| `run start --chain` | 人类入口：创建简单命令链 Session，默认派发第一步 |
| `run start --no-dispatch` | 只创建链，不派发第一步 |
| `run done` | 人类入口：check + complete 当前 Run，只返回下一步建议 |
| `run edit` | 中途修改未来 chain step，不创建新 Session |
| `session create --chain` | 简单建链；命令名直接传入 |
| `session create --chain-file` | 高级 JSON 链定义入口 |
