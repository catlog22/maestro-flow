...
**A_AUDIT** — 6-dimension parallel audit via CSV wave.

**Step 1 — Generate `ui-audit.csv`:**

```csv
id,dimension,focus,target,status,findings_json,error
"1","visual_hierarchy","Spacing, typography scale, color contrast, alignment, whitespace, visual weight","{target}","","",""
"2","interaction_states","Hover, focus, active, disabled, loading, error, empty, selected states","{target}","","",""
"3","accessibility","WCAG AA contrast, focus management, aria labels, keyboard nav, screen reader","{target}","","",""
"4","responsiveness","Breakpoints, overflow, touch targets, fluid typography, container queries","{target}","","",""
"5","micro_interactions","Transitions, animations, feedback indicators, loading states, progress","{target}","","",""
"6","edge_cases","Long text truncation, empty data, error states, extreme values, i18n, RTL","{target}","","",""
```

If `--dimensions` subset specified, include only matching rows.

**Step 2 — Execute CSV wave:**

```javascript
MANDATORY, NOT SUBSTITUTABLE by manual Read/Grep:
spawn_agents_on_csv({
  csv_path: `${sessionFolder}/ui-audit.csv`,
  id_column: "id",
  instruction: "You are auditing UI for a specific dimension. Read 'dimension' and 'focus' columns. Scan target component/page files. Return findings as JSON: [{title, severity, file, line, description, suggestion, dimension}].",
  max_concurrency: 6,
  max_runtime_seconds: 3600,
  output_csv_path: `${sessionFolder}/ui-audit-results.csv`,
  output_schema: { id, status, findings_json, error }
})
```
...
**Step 1 — Generate `diverge.csv`:**

```csv
id,category,focus,target,status,ideas_json,error
"1","polish","Shadows, borders, transitions, hover, feedback, empty states, skeleton loading, scroll behavior","{target}","","",""
"2","delight","Motion design, progressive disclosure, smart defaults, contextual hints, celebratory feedback, personality in copy","{target}","","",""
```
...
MANDATORY, NOT SUBSTITUTABLE by manual Read/Grep:
spawn_agents_on_csv({
  csv_path: `${sessionFolder}/diverge.csv`,
  id_column: "id",
  instruction: "You are exploring UI improvements. Read 'category' (polish or delight) and 'focus' for your area. Go beyond defect fixing — find opportunities. Return ideas as JSON: [{idea, category, impact, effort, description, inspiration}].",
  max_concurrency: 2,
  max_runtime_seconds: 3600,
  output_csv_path: `${sessionFolder}/diverge-results.csv`,
  output_schema: { id, status, ideas_json, error }
})
```
