...
2 parallel agents via CSV wave.

Generate `archaeology.csv`:

```csv
id,title,description,files,status,findings,error
"timeline","Git Timeline","Run git log --oneline -20 -- {files} to gather recent commit timeline for the affected files","{files}","","",""
"blame","Git Blame","Run git blame -L {region} on top 3 affected files to identify who changed the relevant code regions","{files}","","",""
```
...
spawn_agents_on_csv({
  ...
  id_column: "id",
  instruction: "You are investigating a bug. Read the 'description' column for your task. Execute the git commands described and analyze the results for relevance to the issue: {issue}. Write findings as a JSON array of relevant commits/changes with risk assessment.",
  max_concurrency: 2,
...
Update understanding.md §2 (Archaeology).
...
1. Generate hypotheses from evidence, ranked [HIGH]/[MEDIUM]/[LOW] → understanding.md §4.
