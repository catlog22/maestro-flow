...
- **PRODUCT.md exists, DESIGN.md missing**: skip to Step 5 and offer to run `/maestro-impeccable document` for DESIGN.md.
...
If teach was invoked as a setup blocker by another command, such as `/maestro-impeccable craft landing page`, pause that command here. Complete teach, re-run the loader, then resume the original command with the freshly loaded context. For craft, resume into shape next; teach creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
...
If the user agrees, delegate to `/maestro-impeccable document` (it auto-detects scan vs seed). Load its reference and follow that flow.

If the user prefers to skip, mention they can run `/maestro-impeccable document` any time later.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `maestro impeccable load-context` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.

If teach was invoked as a blocker by another impeccable command (e.g. the user ran `/maestro-impeccable polish` with no PRODUCT.md), resume that original task now with the fresh context.
