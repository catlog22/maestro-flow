Report and repair drift between this project's Impeccable artifacts and what the Maestro-bundled core reads: PRODUCT.md, DESIGN.md and its `.impeccable/design.json` sidecar, `.impeccable/config.json`, persisted surface briefs, and the design hook.
...
- **Bundled core version.** Impeccable core updates ship with Maestro. Version comparison with a separately published Skill is intentionally outside this command; use `maestro update` only when the user asks to update Maestro.
...
```
node <impeccable-base>/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node <impeccable-base>/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
