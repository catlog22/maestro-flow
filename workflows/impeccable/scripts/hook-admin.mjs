...
const BUNDLED_SCRIPTS_DIR = path.dirname(fileURLToPath(import.meta.url));
const scriptCommand = (name) => `node "${path.join(BUNDLED_SCRIPTS_DIR, name).replaceAll('\\', '/')}"`;
const HOOK_COMMAND = scriptCommand('hook.mjs');
const HOOK_BEFORE_EDIT_COMMAND = scriptCommand('hook-before-edit.mjs');
...
const HOOK_MANIFEST_TARGETS = [
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        ...
        Stop: [stopManifestEntry(HOOK_COMMAND)],
      },
    }),
  },
  {
  ...
  },
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: HOOK_BEFORE_EDIT_COMMAND,
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
