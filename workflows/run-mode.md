...
5. Participant identity is mandatory on every mutation: pass the authorized identity to `--actor <actor-id>` (or `$MAESTRO_ACTOR`); `--participant` defaults to the same authorized identity, and explicit differing values are rejected. `--reason` defaults to `cli:<command>`; repeatable `--evidence <ref>` stays explicit. This is the `participant_identity` capability contract.
6. When `--request-id` is omitted the CLI derives a stable idempotency key from the invocation (command, arguments, and option values) — a verbatim retry replays the original receipt. Supply an explicit unique `--request-id` when an identical flag set must legitimately re-execute. After every mutation, replace the cached revision with the returned fence before issuing another command. A partial locator, stale orchestration revision, changed run revision, or uncertain write result requires canonical status/recovery; never guess missing fields.
...
3. Negotiate capabilities. Resolve an existing compatible Session identity explicitly, or open a new empty Session. For a self-started task use a distinct stable request ID:
   ...
   `maestro session open "<objective>" --id <slug> [--definition-of-done "<text>"] --actor {actor_id} [--evidence <ref> ...] --json`
...
4. Insert a self-started step with its positional domain text, using the exact revision returned by open and a new request ID:
   ...
   `maestro session chain insert --session {session_id} --step-id {step_id} --command <command-name> [--arg "<task text>" ...] --actor {actor_id} [--evidence <ref> ...] --expected-orchestration-revision {open_orchestration_revision} --json`
5. Dispatch the pending chain step with the exact revision returned by insert and another new request ID:
   ...
   `maestro run create <command> "<task text>" --session {session_id} --run {run_id} --step {step_id} [--parent-run <id>] [--retry-of-run <id>] [--goal "<text>"] [--input <ART-id> ...] --actor {actor_id} [--evidence <ref> ...] --expected-orchestration-revision {step_orchestration_revision} --json`
...
maestro run create odyssey-planex "complete the integration plan" --session 20260715-odyssey-planex-auth --run run-planex-1 --step step-planex-1 --goal "complete the integration plan" --actor actor-7 --expected-orchestration-revision 4 --json
```

Never pass raw task prose through `--input`; `--input <ART-id>` is validated as a sealed Artifact from the same Session. A direct `run create` missing `--session`, `--run`, `--step`, `--actor` identity, or the exact orchestration revision is invalid.
...
- The chain is Session-owned. The complete mutation capability set is the `session chain insert|replace|update|skip` surface, plus decision gates recorded by `run decide`:
  - `maestro session chain insert --session {session_id} --step-id <id> --command <name> [--arg <value> ...] [--after-step <id>] [--goal-ref <id>] [--stage <name>] [--decision-ref <id>] --actor {actor_id} --expected-orchestration-revision {orchestration_revision} --json`
  - `maestro session chain replace --session {session_id} --step-id <id> --command <name> [--arg <value> ...] --actor {actor_id} --expected-orchestration-revision {orchestration_revision} --json`
  - `maestro session chain update --session {session_id} --step-id <id> [--command <name>] [--arg <value> ...] [--goal-ref <id>] [--stage <name>] [--decision-ref <id>] --actor {actor_id} --expected-orchestration-revision {orchestration_revision} --json` (omitted fields are preserved; clearing and decision-gate retargeting are unsupported)
  - `maestro session chain skip --session {session_id} --step-id <id> --actor {actor_id} --evidence <ref> --expected-orchestration-revision {orchestration_revision} --json` (skip requires evidence via `--evidence`)
...
- A Run that must stop early is transitioned, not paused: `maestro run transition {run_id} {running|blocked|failed} --session {session_id} --actor {actor_id} --expected-run-revision {run_revision} [--expected-orchestration-revision {orchestration_revision} when status=failed] --json`, or `maestro run cancel {run_id} --session {session_id} --actor {actor_id} --expected-run-revision {run_revision} --expected-orchestration-revision {orchestration_revision} --json`. Cancellation returns the running chain step to pending for a later fenced `maestro run next`; failure leaves the step failed for inspection. There is no Execution lease to release.
...
- When every Run is sealed, every chain step terminal (completed or skipped with evidence), and no open decision gate remains, finish the Session with:
  ...
  - Single Session: `maestro session migrate --session {legacy_session_id} --to-v3 --actor {actor_id} --expected-identity-revision {legacyIdentityRevision} --expected-activity-revision {legacyActivityRevision} --json`
  - Batch: `maestro session migrate --all --to-v3 --actor {actor_id} --expected-revisions '<session revision manifest JSON>' --json`
