...
```
P1 Context Collection → P2 Clarification → P3 Planning → P4 Plan Checking → P4.5 Collision → P5 Confirmation

Mode routing:
  --check <plan-dir>  → Check Mode (P4 only, read-only)
  --revise            → Revise Mode (load → modify → P4)
  --tdd               → TDD Mode: P1 → P2 → P3(TDD task chain) → P4 → P4.5 → P5
...
--gaps                   → load latest-debug gaps, skip P1 exploration, only plan gap fixes
...
## Stage Gates (Create mode, mandatory blocking)

**GATE context-collected (P1 → P2)**
...
**GATE plan-confirmed (P5 → wrap-up)**
...
3. **Confidence scoring**: 5 dims (requirements_coverage, task_quality, dependency_correctness, estimation_accuracy, collision_safety) × factors (completeness .30, specificity .25, structural_validity .20, user_validation .15, consistency .10). Re-score each round. Quality mechanisms: Pressure Pass (mandatory before P4.5, verifies the highest-complexity task's read_first/convergence/action), Devil's Advocate (coverage>0.7 → "implicit requirement?"), Scope Minimizer, Stall Detection
4. **Readiness Gate** (blocks P4.5): blocking conditions — coverage<40% | a task missing read_first/convergence | no pressure pass | circular dependency. If blocked → AskUserQuestion: revise / ignore risk and continue (record residual_risks)
...
2. **AskUserQuestion** (skip and auto-pass if `config.gates.confirm_plan == false`): Execute now / Verify plan quality (re-run P4 more strictly) / Just view / Modify (change specified tasks, back to P4)

---

## Output Skeleton

**outputs/plan.json** (primary, alias `current-plan`):
```json
{
  "objective": "",
  "requirement_refs": [],
...
**outputs/waves.json** (schema `execution-waves/1.0`), **outputs/dependency-graph.json** (`dependency-graph/1.0`), **outputs/collision-report.json** (`collision-report/1.0`). If a plan check runs, write `outputs/evidence/plan-check.json` (role evidence).
...
5. **Re-run P4** + collision, show results and confirm
...
| exploration agent failed | record, continue with existing exploration; mark plan [LOW CONFIDENCE] |
| planner produced invalid JSON | retry once, then abort with details |
| plan-checker exceeded 3 rounds | accept the plan with warnings, record in index; mark [LOW CONFIDENCE] |
