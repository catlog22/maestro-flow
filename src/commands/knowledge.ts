...
export function registerKnowledgeCommand(program: Command): void {
  ...
  knowledge
    ...
    .option('--evidence <refs>', 'Comma-separated evidence references; file anchors support :line, :line:column, and :start-end (required for session-source candidates)')
    ...
    .option('--session <maestro-session-id>', 'Existing Maestro Workflow Session ID (not a Pi chat/session-history ID; usable without --run)')
