...
SWIFT_CLASS("_TtC6MTAIGC13MTAIGCRequest")
@interface MTAIGCRequest : NSObject
@property (nonatomic, copy) NSString *taskID;
- (void)doThing:(id)value withError:(NSError **)error;
@end

@interface MTAIGCRequest (SWIFT_EXTENSION(MTAIGC))
...
describe.skipIf(!isTreeSitterAvailable())('objcExtractor structural facts', () => {
  ...
  beforeAll(async () => {
    const [local, external, generated, objcxx] = await Promise.all([
      ...
      runner.extract(GENERATED_HEADER_SOURCE, 'objc', '/project/Pods/MTAIGC-Swift.h'),
  ...
  it('handles generated Swift header macros without collapsing category identity', () => {
    expect(results.generated.symbols.map(symbol => symbol.qualifiedName)).toEqual(
      expect.arrayContaining([
        'MTAIGCRequest',
        'MTAIGCRequest.taskID',
        'MTAIGCRequest.doThing:withError:',
        'MTAIGCRequest (SWIFT_EXTENSION(MTAIGC))',
        'MTAIGCRequest (SWIFT_EXTENSION(MTAIGC)).description',
      ]),
    );
    expect(results.generated.symbols.find(symbol => symbol.qualifiedName === 'MTAIGCRequest')?.metadata)
      .toMatchObject({ swiftRuntimeIdentity: '_TtC6MTAIGC13MTAIGCRequest' });
  });
