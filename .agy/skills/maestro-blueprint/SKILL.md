...
<interview_protocol>
Interview the user relentlessly until shared understanding is reached. Active only in interactive mode; skip when `-y/--yes`, `-c/--continue`, or input is already specific (clear idea + scope).
...
- Never ask what code can verify — resolve via `state.json`, existing artifacts, `maestro spec load`, or codebase exploration.
- Walk the decision dependency tree: scope → spec type → focus areas → requirement priorities. Do not open the next branch until the current one is settled.
